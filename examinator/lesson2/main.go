...
func run(strArgs []string) int {
	...
	if err := ex.Download("ex2-1.html", saveFile); err != nil {
		printf("error downloading ex2-1.html: %v", err)
		return exitCodeError
	}

	if err := ex.Download("ex2-2.html", saveFile); err != nil {
		printf("error downloading ex2-2.html: %v", err)
		return exitCodeError
	}

	if err := ex.Download("ex2-2.js", saveFile); err != nil {
		printf("error downloading ex2-2.js: %v", err)
		return exitCodeError
	}

	if err := ex.Download("ex2-3.html", saveFile); err != nil {
		printf("error downloading ex2-3.html: %v", err)
		return exitCodeError
	}

	if err := ex.Download("ex2-3.js", saveFile); err != nil {
		printf("error downloading ex2-3.js: %v", err)
		return exitCodeError
	}
